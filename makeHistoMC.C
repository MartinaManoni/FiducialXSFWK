...
void FitHisto(TH1D* _hist, TString name)
{
...
void makeHistoMC(){
  ...
  string proc = "DY_2022";
  double lumi;
  fpath = "/eos/user/m/mbonanom/run3_trees/DY_2022.root";
  lumi = 26.6728;
  ...
  for(int i = 2; i < eta_cut.size() ; i++){
      ...
      auto hist_ele = rdf.Filter(Form("Z1Mass>=80 && Z1Mass<=105 && Z1Flav==-121 && LepEta[0] < %f && LepEta[0] > %f && LepEta[1] < %f && LepEta[1] > %f", high_eta, low_eta, high_eta, low_eta)).Define("weight", Form("1000 * (6225.4/5558.0) * overallEventWeight * %f / %f", lumi, gen_sumWeights)).Histo1D({"Z1Mass", "Z1Mass", 20, 80., 105.},"Z1Mass", "weight");

      auto hist_ele_smearUp = ext_df.Filter(Form("ZMass_SmearUp>=80 && ZMass_SmearUp<=105 && Z1Flav==-121 && LepEta[0] < %f && LepEta[0] > %f && LepEta[1] < %f && LepEta[1] > %f", high_eta, low_eta, high_eta, low_eta)).Define("weight", Form("1000 * (6225.4/5558.0) * overallEventWeight * %f / %f", lumi, gen_sumWeights)).Histo1D({"ZMass_SmearUp", "ZMass_SmearUp", 20, 80., 105.},"ZMass_SmearUp", "weight");

      auto hist_ele_smearDn = ext_df.Filter(Form("ZMass_SmearDn>=80 && ZMass_SmearDn<=105 && Z1Flav==-121 && LepEta[0] < %f && LepEta[0] > %f && LepEta[1] < %f && LepEta[1] > %f", high_eta, low_eta, high_eta, low_eta)).Define("weight", Form("1000 * (6225.4/5558.0) * overallEventWeight * %f / %f", lumi, gen_sumWeights)).Histo1D({"ZMass_SmearDn", "ZMass_SmearDn", 20, 80., 105.},"ZMass_SmearDn", "weight");

      auto hist_ele_scaleUp = ext_df.Filter(Form("ZMass_ScaleUp>=80 && ZMass_ScaleUp<=105 && Z1Flav==-121 && LepEta[0] < %f && LepEta[0] > %f && LepEta[1] < %f && LepEta[1] > %f", high_eta, low_eta, high_eta, low_eta)).Define("weight", Form("1000 * (6225.4/5558.0) * overallEventWeight * %f / %f", lumi, gen_sumWeights)).Histo1D({"ZMass_ScaleUp", "ZMass_ScaleUp", 20, 80., 105.},"ZMass_ScaleUp", "weight");

      auto hist_ele_scaleDn = ext_df.Filter(Form("ZMass_ScaleDn>=80 && ZMass_ScaleDn<=105 && Z1Flav==-121 && LepEta[0] < %f && LepEta[0] > %f && LepEta[1] < %f && LepEta[1] > %f", high_eta, low_eta, high_eta, low_eta)).Define("weight", Form("1000 * (6225.4/5558.0) * overallEventWeight * %f / %f", lumi, gen_sumWeights)).Histo1D({"ZMass_ScaleDn", "ZMass_ScaleDn", 20, 80., 105.},"ZMass_ScaleDn", "weight");
      ...
      FitHisto(hist_ele.GetPtr(), proc+"_ele_"+s_eta_cut.c_str());
      FitHisto(hist_ele_smearUp.GetPtr(), proc+"_ele_smearUp_"+s_eta_cut.c_str());
      FitHisto(hist_ele_smearDn.GetPtr(), proc+"_ele_smearDn_"+s_eta_cut.c_str());
      FitHisto(hist_ele_scaleUp.GetPtr(), proc+"_ele_scaleUp_"+s_eta_cut.c_str());
      FitHisto(hist_ele_scaleDn.GetPtr(), proc+"_ele_scaleDn_"+s_eta_cut.c_str());
  }
  histoMC->Close();
