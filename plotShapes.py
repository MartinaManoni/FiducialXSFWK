...
def parseOptions():
    ...
    parser.add_option('',   '--year',  dest='YEAR',  type='string',default='2022EE',   help='Year -> 2016 or 2017 or 2018 or Full')
...
def plotAsimov_sim(modelName, physicalModel, obsName, fstate, observableBins, recobin):
    ...
    mass = w_asimov.var("CMS_zz4l_mass")
    mass= mass.frame(RooFit.Bins(30))

    massVar = w_asimov.var("CMS_zz4l_mass")
    print("massVar", massVar)
    if massVar is None:
        raise RuntimeError("CMS_zz4l_mass not found in workspace")
    ...
    if (fstate=="4l"):
        ...
        cat_val = CMS_channel.getLabel()
        print("CMS_channel category label:", cat_val)

        # Get the corresponding PDF:
        pdf = sim.getPdf(cat_val)
        print("Retrieved PDF from sim:", pdf)

        # Check if it contains CMS_zz4l_mass:
        if pdf:
            obs = pdf.getObservables(data)
            print("Observables in selected PDF:")
            obs.Print("v")
        else:
            print("No PDF found for category", cat_val)'''
    ...
    ##### ------------------------ Shapes ------------------------ #####
    print("-------Shapes----------")
    #print("cat_val", cat_val)    # label of the currently‑selected slice
    #cat_tokens = cat_val.split("_")
    #fstate_cat = cat_tokens[-2][3:]  # 'cat4e' → '4e'
    #year_cat   = cat_tokens[-1]  
