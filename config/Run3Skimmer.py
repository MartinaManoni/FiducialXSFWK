...
df_SR = ( df.Filter('bestCandIdx>=0').Define("ZZMass", "ZZCand_mass[bestCandIdx]") ## Dummy
                                    ...
                                    .Define('EventNumber', "event")
                                    .Define('LumiNumber', "luminosityBlock")
                                    #.Define('Leptons_pt_SmearUp', "concatenate(Electron_smearUp_pt,Muon_smearUp_pt)")
                                    #.Define('Leptons_pt_SmearDn', "concatenate(Electron_smearDn_pt,Muon_smearDn_pt)")
                                    #.Define('Leptons_pt_ScaleUp', "concatenate(Electron_scaleUp_pt,Muon_scaleUp_pt)")
                                    #.Define('Leptons_pt_ScaleDn', "concatenate(Electron_scaleDn_pt,Muon_scaleDn_pt)")
                                    ...
                                    #.Define('LepPt_ScaleUp', "std::vector<float> LepPt_ScaleUp{Leptons_pt_ScaleUp[ZZCand_Z1l1Idx[bestCandIdx]], Leptons_pt_ScaleUp[ZZCand_Z1l2Idx[bestCandIdx]], Leptons_pt_ScaleUp[ZZCand_Z2l1Idx[bestCandIdx]], Leptons_pt_ScaleUp[ZZCand_Z2l2Idx[bestCandIdx]]}; return LepPt_ScaleUp")
                                    #.Define('LepPt_ScaleDn', "std::vector<float> LepPt_ScaleDn{Leptons_pt_ScaleDn[ZZCand_Z1l1Idx[bestCandIdx]], Leptons_pt_ScaleDn[ZZCand_Z1l2Idx[bestCandIdx]], Leptons_pt_ScaleDn[ZZCand_Z2l1Idx[bestCandIdx]], Leptons_pt_ScaleDn[ZZCand_Z2l2Idx[bestCandIdx]]}; return LepPt_ScaleDn")
                                    #.Define('LepPt_SmearDn', "std::vector<float> LepPt_SmearDn{Leptons_pt_SmearDn[ZZCand_Z1l1Idx[bestCandIdx]], Leptons_pt_SmearDn[ZZCand_Z1l2Idx[bestCandIdx]], Leptons_pt_SmearDn[ZZCand_Z2l1Idx[bestCandIdx]], Leptons_pt_SmearDn[ZZCand_Z2l2Idx[bestCandIdx]]}; return LepPt_SmearDn")
                                    #.Define('LepPt_SmearUp', "std::vector<float> LepPt_SmearUp{Leptons_pt_SmearUp[ZZCand_Z1l1Idx[bestCandIdx]], Leptons_pt_SmearUp[ZZCand_Z1l2Idx[bestCandIdx]], Leptons_pt_SmearUp[ZZCand_Z2l1Idx[bestCandIdx]], Leptons_pt_SmearUp[ZZCand_Z2l2Idx[bestCandIdx]]}; return LepPt_SmearUp")
                                    ...
                                    #Leptons Systematics 
                                    #.Define('Muon_SF', 'Muon_dataMC')
                                    #.Define('Muon_SFUnc', 'Muon_dataMCUnc')
                                    #.Define('Electron_SF', 'Electron_dataMC')
                                    #.Define('Electron_SFUnc', 'Electron_dataMCUnc')
                                    # tagging 
                                    #.Define('Jet_btagPNetB_', 'Jet_btagPNetB')


                                    #.Define('Muon_SF', "return std::vector<float>{Muon_dataMC[ZZCand_Z1l1Idx[bestCandIdx]], Muon_dataMC[ZZCand_Z1l2Idx[bestCandIdx]], Muon_dataMC[ZZCand_Z2l1Idx[bestCandIdx]], Muon_dataMC[ZZCand_Z2l2Idx[bestCandIdx]]};")
                                    #.Define('Muon_SFUnc', "return std::vector<float>{Muon_dataMCUnc[ZZCand_Z1l1Idx[bestCandIdx]], Muon_dataMCUnc[ZZCand_Z1l2Idx[bestCandIdx]], Muon_dataMCUnc[ZZCand_Z2l1Idx[bestCandIdx]], Muon_dataMCUnc[ZZCand_Z2l2Idx[bestCandIdx]]};")

                                    #.Define('Electron_SF', "return std::vector<float>{Electron_dataMC[ZZCand_Z1l1Idx[bestCandIdx]], Electron_dataMC[ZZCand_Z1l2Idx[bestCandIdx]], Electron_dataMC[ZZCand_Z2l1Idx[bestCandIdx]], Electron_dataMC[ZZCand_Z2l2Idx[bestCandIdx]]};")
                                    #.Define('Electron_SFUnc', "return std::vector<float>{Electron_dataMCUnc[ZZCand_Z1l1Idx[bestCandIdx]], Electron_dataMCUnc[ZZCand_Z1l2Idx[bestCandIdx]], Electron_dataMCUnc[ZZCand_Z2l1Idx[bestCandIdx]], Electron_dataMCUnc[ZZCand_Z2l2Idx[bestCandIdx]]};")
                                    
                                    #.Define("ZlIdx", "std::vector<short>{ZZCand_Z1l1Idx[bestCandIdx], ZZCand_Z1l2Idx[bestCandIdx], ZZCand_Z2l1Idx[bestCandIdx], ZZCand_Z2l2Idx[bestCandIdx]}")
                                    #.Define("Electron_SF", "std::vector<float> out; std::vector<short> ids = LepLepId; for (int i = 0; i < 4; i++) { if (std::abs(ids[i]) == 11) { out.push_back(Electron_dataMC[ZlIdx[i]]); } } return out;")
                                    #.Define("Electron_SFUnc", "std::vector<float> out; std::vector<short> ids = LepLepId; for (int i = 0; i < 4; i++) { if (std::abs(ids[i]) == 11) { out.push_back(Electron_dataMCUnc[ZlIdx[i]]); } } return out;")
                                    #.Define("Muon_SF", "std::vector<float> out; std::vector<short> ids = LepLepId; for (int i = 0; i < 4; i++) { if (std::abs(ids[i]) == 13) { out.push_back(Muon_dataMC[ZlIdx[i]]); } } return out;")
                                    #.Define("Muon_SFUnc", "std::vector<float> out; std::vector<short> ids = LepLepId; for (int i = 0; i < 4; i++) { if (std::abs(ids[i]) == 13) { out.push_back(Muon_dataMCUnc[ZlIdx[i]]); } } return out;")
                                    #.Define("Electron_SF", "std::vector<float> outSF; std::vector<int> idxs{ZZCand_Z1l1Idx[bestCandIdx],ZZCand_Z1l2Idx[bestCandIdx],ZZCand_Z2l1Idx[bestCandIdx],ZZCand_Z2l2Idx[bestCandIdx]}; std::vector<short> flavs{static_cast<short>(Z1Flav), static_cast<short>(Z1Flav), static_cast<short>(Z2Flav), static_cast<short>(Z2Flav)}; for (size_t i=0;i<4;++i){if(abs(flavs[i])==11) outSF.push_back(Electron_dataMC[idxs[i]]);} return outSF;")
                                    #.Define("Electron_SFUnc", "std::vector<float> outSF; std::vector<int> idxs{ZZCand_Z1l1Idx[bestCandIdx],ZZCand_Z1l2Idx[bestCandIdx],ZZCand_Z2l1Idx[bestCandIdx],ZZCand_Z2l2Idx[bestCandIdx]}; std::vector<short> flavs{static_cast<short>(Z1Flav), static_cast<short>(Z1Flav), static_cast<short>(Z2Flav), static_cast<short>(Z2Flav)}; for (size_t i=0;i<4;++i){if(abs(flavs[i])==11) outSF.push_back(Electron_dataMCUnc[idxs[i]]);} return outSF;")
                                    #.Define("Muon_SF", "std::vector<float> outSF; std::vector<int> idxs{ZZCand_Z1l1Idx[bestCandIdx],ZZCand_Z1l2Idx[bestCandIdx],ZZCand_Z2l1Idx[bestCandIdx],ZZCand_Z2l2Idx[bestCandIdx]}; std::vector<short> flavs{static_cast<short>(Z1Flav), static_cast<short>(Z1Flav), static_cast<short>(Z2Flav), static_cast<short>(Z2Flav)}; for (size_t i=0;i<4;++i){if(abs(flavs[i])==13) outSF.push_back(Muon_dataMC[idxs[i]]);} return outSF;")
                                    #.Define("Muon_SFUnc", "std::vector<float> outSF; std::vector<int> idxs{ZZCand_Z1l1Idx[bestCandIdx],ZZCand_Z1l2Idx[bestCandIdx],ZZCand_Z2l1Idx[bestCandIdx],ZZCand_Z2l2Idx[bestCandIdx]}; std::vector<short> flavs{static_cast<short>(Z1Flav), static_cast<short>(Z1Flav), static_cast<short>(Z2Flav), static_cast<short>(Z2Flav)}; for (size_t i=0;i<4;++i){if(abs(flavs[i])==13) outSF.push_back(Muon_dataMCUnc[idxs[i]]);} return outSF;")
...
if not _SKIPZL:
    df_ZL = ( df.Filter('ZLCand_lepIdx>-1').Define("ZZMass", "0") ## Dummy
                                           ...
                                           .Define('Leptons_missinghit', "concatenate(Electron_lostHits, Muon_lostHits)")
                                           ## Variable miniAOD-style
                                           #.Define('LepPt_ScaleUp', "std::vector<float> LepPt_ScaleUp{Leptons_pt_ScaleUp[ZCand_l1Idx[bestZIdx]], Leptons_pt_ScaleUp[ZCand_l2Idx[bestZIdx]], Leptons_pt_ScaleUp[ZLCand_lepIdx]}; return LepPt_ScaleUp")
                                           #.Define('LepPt_ScaleDn', "std::vector<float> LepPt_ScaleDn{Leptons_pt_ScaleDn[ZCand_l1Idx[bestZIdx]], Leptons_pt_ScaleDn[ZCand_l2Idx[bestZIdx]], Leptons_pt_ScaleDn[ZLCand_lepIdx]}; return LepPt_ScaleDn")
                                           #.Define('LepPt_SmearDn', "std::vector<float> LepPt_SmearDn{Leptons_pt_SmearDn[ZCand_l1Idx[bestZIdx]], Leptons_pt_SmearDn[ZCand_l2Idx[bestZIdx]], Leptons_pt_SmearDn[ZLCand_lepIdx]}; return LepPt_SmearDn")
                                           #.Define('LepPt_SmearUp', "std::vector<float> LepPt_SmearUp{Leptons_pt_SmearUp[ZCand_l1Idx[bestZIdx]], Leptons_pt_SmearUp[ZCand_l2Idx[bestZIdx]], Leptons_pt_SmearUp[ZLCand_lepIdx]}; return LepPt_SmearUp")
    ...
    opts.fMode = 'UPDATE'
    df_ZL.Snapshot('CRZLTree/candTree', outFileName, list(vars), opts)
