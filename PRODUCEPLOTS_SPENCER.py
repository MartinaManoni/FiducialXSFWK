...
def parseOptions():
    ...
    parser.add_option('',   '--obsName',dest='OBSNAME',    type='string',default='phi1',   help='Name of the observalbe, supported: "inclusive", "pT", "eta", "Njets"')
    parser.add_option('',   '--obsBins',dest='OBSBINS',    type='string',default='|-3.14159265359|-2.35619449019|-1.57079632679|-0.785398163397|0.0|0.785398163397|1.57079632679|2.35619449019|3.14159265359|',   help='Bin boundaries for the diff. measurement separated by "|", e.g. as "|0|50|100|", use the defalut if empty string')
    ...
    for obsBin in range(nBins-1):
        ...
        for channel in ['4e','4mu','2e2mu']:
            ...
            if (obsName.startswith("mass4l")):
                ...
                #(unc_qcd_ggH_hi*ggH_xsBR*acc_NNLOPS['ggH125_NNLOPS_'+channel+'_'+obsName+'_genbin'+str(obsBin)+'_recobin'+str(obsBin)])**2
                total_NNLOunc_fs_minloHJ_lo += 0.0
                #(unc_qcd_ggH_lo*ggH_xsBR*acc_NNLOPS['ggH125_NNLOPS_'+channel+'_'+obsName+'_genbin'+str(obsBin)+'_recobin'+str(obsBin)])**2
                total_NNLOunc_fs_aMC_hi += 0.0
                #(unc_qcd_ggH_hi *ggH_xsBR*acc['ggH125__'+channel+'_'+obsName+'_genbin'+str(obsBin)+'_recobin'+str(obsBin)])**2
                total_NNLOunc_fs_aMC_lo += 0.0
                #(unc_qcd_ggH_lo*ggH_xsBR*acc['ggH125_'+channel+'_'+obsName+'_genbin'+str(obsBin)+'_recobin'+str(obsBin)])**2
            else:
                total_NNLOunc_fs_minloHJ_hi += 0.0
...
#                                           *ggH_xsBR*acc['ggH125_'+channel+'_'+obsName+'_genbin'+str(obsBin)+'_recobin'+str(obsBin)])**2
            ...
            #NNLO
            if (obsName.startswith("mass4l")):
                obsUnc_pdf_ggH_hi = 0.0 #unc_pdf_ggH_hi
                obsUnc_pdf_ggH_lo = 0.0 #unc_pdf_ggH_lo
            else:
...
#            ggH_aMC_NLOunc_lo[obsBin]+=sqrt(total_NLOunc_fs_aMC_lo)
    ...
    if (obsName.startswith("mass4l")):
        for channel in ['2e2mu','4mu','4e']:
            ...
            total_NNLOunc_fs_aMC_hi = (unc_br*(XH_fs+ggH_xsBR*acc['ggH125_'+channel+'_'+obsName+'_genbin0_recobin0']))**2
            total_NNLOunc_fs_aMC_lo = (unc_br*(XH_fs+ggH_xsBR*acc['ggH125_'+channel+'_'+obsName+'_genbin0_recobin0']))**2
            total_NNLOunc_fs_aMC_hi += (unc_acc*(XH_fs+ggH_xsBR*acc['ggH125_'+channel+'_'+obsName+'_genbin0_recobin0']))**2
            total_NNLOunc_fs_aMC_lo += (unc_acc*(XH_fs+ggH_xsBR*acc['ggH125_'+channel+'_'+obsName+'_genbin0_recobin0']))**2
    ...
    if(opt.SETLOG):
        ...
        elif obsName == 'mass4l': dummy.SetMaximum(6)
        elif obsName == 'rapidity4l': dummy.SetMaximum(4) #QUI
    ...
    if obsName == 'pT4l':
        ...
        dummy2.GetYaxis().SetRangeUser(0, 2) #HERE change range of the ratio plot
    elif obsName == 'costhetaZ2':
